...
export default function MerchantDashboardPage() {
  const totalReceived = mockPaymentLinks.reduce((sum, link) => sum + link.totalReceived, 0);
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-5 p-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="card"><p className="text-white/70">Active links</p><p className="mt-1 text-2xl font-bold">{mockPaymentLinks.filter((link) => link.status === 'active').length}</p></div>
        <div className="card"><p className="text-white/70">Transactions</p><p className="mt-1 text-2xl font-bold">{mockTransactions.length}</p></div>
      </div>
